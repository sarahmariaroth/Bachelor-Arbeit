...
def denormalize(tensor):
    # (tensor * std) + mean
    denormalized_tensor = tensor * torch.tensor([0.5, 0.5, 0.5]).view(3, 1, 1) + torch.tensor([0.5, 0.5, 0.5]).view(3, 1, 1)
...
class Experiment(object):
    ...
    def evaluate(self):
        ...
        psnr = compare_psnr(y.cpu().numpy(), d.cpu().numpy(), data_range=2.0)
